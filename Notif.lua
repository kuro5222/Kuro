...
local library = {accent = Color3.fromRGB(255, 255, 127), noti1 = {}, noti2 = {}}; do
    ...
    -- Notify1 Function (Top-Left Notifications)
    ...
    -- Notify2 Function (Top-Center Notifications)
    function library:notify2(name, time)
        local name, time = name or 'Notify2', time or 1

        local label = Instance.new('TextLabel')
        label.AnchorPoint = Vector2.new(0.5, 0) -- Align top-center
        label.Parent = screen_gui
        label.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        label.BackgroundTransparency = 1.000
        label.BorderColor3 = Color3.fromRGB(0, 0, 0)
        label.BorderSizePixel = 0
        label.Position = UDim2.new(0.5, 0, 0.1, #self.noti2 * 30) -- Set top-center position
        ...
        task.delay(time, function()
            ...
            for i, v in pairs(self.noti2) do 
                TweenService:Create(v.label, TweenInfo.new(.25), {Position = UDim2.new(0.5, 0, 0.1, (i - 1) * 30)}):Play() -- Adjust stacking positions
            end
        end)
